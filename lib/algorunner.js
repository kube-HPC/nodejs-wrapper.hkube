...
class Algorunner {
    ...
    _registerToCommunicationEvents() {
        ...
        this._wsc.on(messages.incoming.message, async (options) => {
            const sendMessage = (message, flowName, flowPattern) => {
                this._hkubeApi.sendMessage(message, flowName, flowPattern, options.sendMessageId);
            };
            ...
            this._wsc.send({ command: messages.outgoing.doneMessage, data: { sendMessageId: options.sendMessageId } });
        });
    }
