...
class Algorunner {
    ...
    async _handleResponseV2(algorithmData, jobId, taskId, nodeName, info) {
        ...
        if (this._isStreaming) {
            this._hkubeApi.stopStreaming(false);
        }
    ...
    async _stop(options) {
        try {
            if (this._stopping) {
                console.log('Got stop command while already stopping');
                return;
            }
            this._stopping = true;
            ...
            if (this._isStreaming) {
                if (!options.forceStop) {
                    console.log('entering stopping soon');
                    let stoppingState = true;
                    ...
                    this._hkubeApi.stopStreaming(true);
                }
            }
