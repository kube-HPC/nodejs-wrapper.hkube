...
const { StdOutRedirect } = require('./utils/stdOutRedirect');
...
class Algorunner {
    ...
    _loadAlgorithm(algorithm) {
        try {
            ...
            this._logDebug = algorithm.logs || false;
            log.info('algorithm code loaded');
        }
        catch (e) {
            this._loadAlgorithmError = e.message;
    ...
    async _init(options) {
        ...
        try {
            if (this._logDebug) {
                redirect = new StdOutRedirect();
                redirect.on('print', (data) => {
                    this._sendCommand({ command: messages.outgoing.logData, data });
                });
            }
