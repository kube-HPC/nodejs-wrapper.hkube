...
const DataServer = require('./communication/dataServer');
...
class Algorunner {
    ...
    initDataServer(options) {
        const { host, port, binary } = options.algorithmDiscovery;
        this._algorithmDiscovery = { host, port };
        this._dataServer = new DataServer({ port, binary });
    }
    ...
    async _start() {
        try {
            ...
            if (Object.keys(storage).length > 0) {
                this._input.input = await storageAdapter.input(this._input.input, storage);
            }

            const output = await this._algorithm.start(this._input);
            const metadata = storageAdapter.createMetadata({ nodeName, data: output, savePaths: info.savePaths });
            const path = storageAdapter.createStoragePath({ jobId, taskId });

            this._state = 'storing';
            this._dataServer.setSendingState(taskId, output);
            this._sendCommand({ command: messages.outgoing.storing, data: { discovery: this._algorithmDiscovery, storageInfo: { path }, metadata } });
