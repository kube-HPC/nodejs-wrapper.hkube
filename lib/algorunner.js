...
class Algorunner {
    ...
    async _handleResponseV2(algorithmData, jobId, taskId, nodeName, info) {
        const { header, payload: encodedData } = dataAdapter._encoding.encodeHeaderPayload(algorithmData);
        ...
        let inCache = false;
        if (this._dataServer && info.savePaths) {
            inCache = this._dataServer.setSendingState(taskId, header, encodedData);
        }
        if (inCache) {
