...
class Algorunner {
    ...
    connectToWorker(options) {
        try {
            ...
            const { url, protocol, host, port, binary, flags = [] } = config;
            this._url = url || `${protocol}://${host}:${port}?flags=${flags.join(',')}`;
    ...
    async _start() {
        try {
            ...
            const storageInfo = await storageAdapter.output({ output, ...this._input });

            this._sendCommand({ command: messages.outgoing.done, data: storageInfo });
        }
        catch (error) {
            this._sendError(error);
        }
