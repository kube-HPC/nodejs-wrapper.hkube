...
class Algorunner {
    ...
    async connectToWorker(options) {
        try {
            ...
            this._hkubeApi = new CodeApi(this._wsc, this, dataAdapter, this._options.storageMode);
    ...
    _registerToCommunicationEvents() {
        ...
        this._wsc.on(messages.incoming.subPipelineDone, options => this._hkubeApi.algorithmExecutionDone(options));
        this._wsc.on(messages.incoming.subPipelineError, options => this._hkubeApi.algorithmExecutionDone(options));
        this._wsc.on(messages.incoming.subPipelineStopped, options => this._hkubeApi.algorithmExecutionDone(options));
    }
