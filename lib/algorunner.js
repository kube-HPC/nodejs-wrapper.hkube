...
class Algorunner {
    constructor() {
        ...
        this._hkubeApi = null;
        this._streamingManager = new StreamingManager(e => this._sendError(e));
    ...
    async connectToWorker(options) {
        try {
            ...
            this._hkubeApi = new CodeApi(this._wsc, this, dataAdapter, this._options.storageMode, this._streamingManager);
    ...
    _discoveryUpdate(discovery) {
        console.log(`Got discovery update ${discovery}`);
        const messageListenerConfig = { encoding: this._options.discovery.encoding };
        this._streamingManager.setupStreamingListeners(messageListenerConfig, discovery, this._nodeName);
    }

    _setupStreamingProducer() {
        this._streamingManager.setupStreamingProducer({
            onStatistics: s => this._sendCommand(messages.outgoing.streamingStatistics, s),
            producerConfig: this._options.discovery.streaming,
            nextNodes: this._childs,
    ...
    async _exit(options) {
        ...
        try {
            ...
            this._dataServer && this._dataServer.close();
