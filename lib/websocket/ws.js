...
class AlgorithmWS extends EventEmitter {
    constructor(options) {
        ...
        this._setSerialization(options);
        this._connect();
    }

    _setSerialization(options) {
        this._parse = options.binary ? binaryDecode : JSON.parse;
        this._stringify = options.binary ? binaryEncode : JSON.stringify;
    }
    ...
    _handleMessages() {
        this._socket.on('message', (message) => {
            const payload = this._parse(message);
