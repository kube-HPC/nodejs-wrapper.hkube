...
const debug = (debugUrl, callbacks, { algorithmName, hkubeUrl, pathToAlgorithm } = {}, logs = false) => {
    ...
    if (callbacks) {
        algorunner.loadAlgorithmCallbacks({ ...callbacks, logs });
    }
    else {
        algorunner.loadAlgorithm({ ...config.algorithm, logs });
    }
