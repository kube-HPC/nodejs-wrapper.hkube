...
class StorageAdapter {
    ...
    async wrappedGetFromStorage(info, trace) {
        if (this.enableCache) {
            const cached = this.storageCache[info.storageInfo.path];
            if (cached) {
                return cached;
            }
        }
        let data;
        if (info.discovery) {
            data = await this.getFromPeer(info);
        }
        if (!data) {
            data = await storageManager.get(info.storageInfo, trace);
        }
        if (this.enableCache) {
            this.storageCache[info.storageInfo.path] = data;
        }
        return data;
    }
