...
class StorageAdapter {
        ...
        const promiseDataExtractors = Object.entries(flatObj).map(async ([k, v]) => {
            if (typeof v === 'string' && v.startsWith('$$')) {
                ...
                if (Array.isArray(link.storageInfo)) {
                    ...
                    data = await this.wrappedGetFromStorage(link, tracerStart);
                    if (link.path) {
    ...
    async wrappedGetFromStorage(options, trace) {
        let data;
        let isStorage = false;
        if (options.discovery) {
            data = await this.getFromPeer(options);
        }
        if (!data) {
            data = await this.getFromStorage(options, trace);
            isStorage = true;
        }
        return data;
    }

    async getFromPeer(options) {
        let { path } = options;
        const { taskId, index } = options;
        const { port, host } = options.discovery;
        if (index) {
            path = (path && `${path}.${index}`) || `${index}`;
        }
        const dataRequest = new DataRequest({ port, host, taskId, dataPath: path, binary: true });
        const response = await dataRequest.invoke();
        return response.data;
    }

    async getFromStorage(options, trace) {
        if (this.enableCache) {
            const cached = this.storageCache[options.storageInfo.path];
            if (cached) {
                return cached;
            }
        }
        const data = await storageManager.get(options.storageInfo, trace);
        if (this.enableCache) {
